...
package pruebas_productos as  
    ...
    procedure actualizar(productoid_in integer, nombre_in number, nombre_esperado number,salida_esperada boolean);
...
package body pruebas_productos as  
    ...
    begin 
        ...
        dbms_output.put_line('Prueba ' || pruebaproductosec.nextval || ': ' || assert_equals(salida,salida_esperada));
        
        exception 
        when others then
            dbms_output.put_line('Prueba ' || pruebaproductosec.nextval || ': ' || assert_equals(false,salida_esperada));
            rollback;        
    end insertar;
    
    procedure actualizar(
        productoid_in integer, nombre_in number, nombre_esperado number,salida_esperada boolean
    ) as
        salida boolean :=true;
        valor integer;
        fila productos%rowtype;
